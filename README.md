...
XolvienはGitHub Actions + Claude Codeの課題を解決する独自のWebアプリケーションです：
...
- 要件明確化フェーズ（プロンプト生成前にClaudeが不明点を質問・複数ラウンド対応）
- 指示入力欄の大型化・ログの起動時自動表示・リサイズ可能スプリットペイン
...
以降の手順はすべて `xolvien/` ディレクトリ内で実行します。
...
Xolvien は AI との通信に Claude Code CLI を使います。ログイン済みの認証情報（`~/.claude/`）をコンテナにマウントして使用するため、APIキーの設定は不要です。Max Plan のサブスクリプションがあれば追加費用なく動作します。
...
docker compose ps
# xolvien-db が "Up (healthy)" になればOK
```
...
4. 指示欄に日本語で簡潔な指示を入力して **「要件を確認する」** をクリック
5. Claude が不明点を質問してくるので回答する（要件が明確な場合は **「スキップしてプロンプトを生成」** も可）
6. 十分な情報が揃うと自動的にプロンプト確認画面へ移行する
7. 確認画面でプロンプトを確認し、不満があれば指摘を書いて **「再生成」**、よければ **「確定して実行」**
8. ログエリアにClaude Code CLIの出力がリアルタイムで表示され、完了後に自動コミットされる
9. タスク詳細画面の **「Git Push」** ボタンをクリックしてGitHubへ保存する
...
```bash
docker build --no-cache -t xolvien-workspace:latest ./docker/workspace/
```
...
```bash
docker rm -f xolvien-task-{タスクID}
```
...
**Q: コンテナが残り続けますが？**
A: タスク削除時に自動削除されます。手動で削除する場合は `docker rm -f xolvien-task-X`
