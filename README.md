...
7. ログエリアにClaude Code CLIの出力がリアルタイムで表示される
8. 完了後、GitHubへpushして保存する：
   ```bash
   docker exec karakuri-task-{タスクID} bash -c "cd /workspace/repo && git add . && git commit -m 'コミットメッセージ' && git push -u origin HEAD"
   ```
