...
- **追加対応（2026-04-26）**: アクションボタンをチャット履歴カード内から入力エリアのフッター上部に移動。`renderActionButtons()` により `selectedStep` に応じてボタンセットを動的に切り替え。「実装」ステップクリック時に入力欄への自動挿入を廃止。システム通知はチャット履歴末尾の `info` エントリとして表示
