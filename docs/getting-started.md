...
**リポジトリ**
コードの保存場所（GitHubのリポジトリ）です。**1つのプログラム = 1つのリポジトリ**が基本です。一度 Xolvien に登録すると、次回からは登録済みのものを選ぶだけです。

**タスク**
ひとつの作業単位です。「翻訳アプリを作る」「ログイン機能を追加する」のように、やりたいことごとに1つ作ります。タスクを作るたびに専用の作業環境（コンテナ）が用意され、**`main` ブランチから新しい作業ブランチが自動で切られます**。

```
リポジトリ A（翻訳アプリ）
  └─ タスク1「翻訳アプリを作る」  → xolvien/1-translation-app ブランチ
  └─ タスク2「デザインを改善する」 → xolvien/2-improve-design  ブランチ

リポジトリ B（電卓アプリ）
  └─ タスク3「電卓アプリを作る」  → xolvien/3-calculator-app  ブランチ
```

タスクごとに `main` から独立したブランチが切られるので、別タスクの作業内容が混入しません。**新しいプログラムを作るときは新しいリポジトリを用意してください。**
