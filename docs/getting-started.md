...
「リポジトリ」とは、コードを保存するGitHubの場所です。

画面には **「既存のリポジトリを選択」** と **「新しいリポジトリを追加」** の2つのタブがあります。

| タブ | いつ使うか |
|---|---|
| **新しいリポジトリを追加** | Karakuriに初めてこのリポジトリを登録するとき |
| **既存のリポジトリを選択** | 過去のタスク作成時にすでに登録済みのリポジトリを使うとき |

**初めて使う場合は「新しいリポジトリを追加」を選びます。**
...
GitHubのリポジトリページを開き、緑色の **「Code」** ボタン → **「SSH」** タブをクリックすると表示されます。
...
> **ブランチ名について：** 空白のままにすると `karakuri/task-{番号}` という名前のブランチが自動的に作成されます。これにより、タスクごとに独立したブランチで作業でき、複数のタスクを並行して進めても競合が起きません。既存のブランチ（例：`feature/login`）に作業を追加したい場合はそのブランチ名を入力してください。
