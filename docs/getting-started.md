...
実装が完了すると、右パネルに自動的にテストケースの確認画面が表示されます。
...
- **テストケースを直接編集したい場合**：テキストエリアを直接書き換えてから **「承認してテスト実行」** をクリックします
- **Claude に修正を依頼したい場合**：**「修正を依頼」** をクリックすると修正内容の入力欄が表示されます。修正してほしい内容を入力して送信してください
...
> **次回以降のセッションでの再開について：** タスク詳細画面を開き直すと、前回テストが完了した状態から自動的に再開されます。単体テストが完了済みの場合は実装確認画面、未完了の場合はテストケース確認画面から再開します。
