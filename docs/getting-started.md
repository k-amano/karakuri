...
**リポジトリ**
コードの保存場所（GitHubのリポジトリ）です。一度 Karakuri に登録すると、次回からは登録済みのものを選ぶだけです。
...
```
リポジトリ（GitHubのURL）
  └─ タスク1「翻訳アプリを作る」  → karakuri/task-1 ブランチ
  └─ タスク2「電卓アプリを作る」  → karakuri/task-2 ブランチ
  └─ タスク3「ログイン機能を追加」 → karakuri/task-3 ブランチ
```
...
#### 2回目以降（このリポジトリをすでに登録済みの場合）

**「既存のリポジトリを選択」** タブを開くと、過去に登録したリポジトリの一覧が表示されます。使いたいリポジトリをリストから選んでください。

> **どちらを選ぶか迷ったら：** 画面を開いたとき「既存のリポジトリを選択」タブに何も表示されていなければ、まだ登録されていないので「新しいリポジトリを追加」を選びます。リストにリポジトリ名が表示されていれば「既存のリポジトリを選択」で選びます。
...
> **ブランチ名について：** 空白のままにすると `karakuri/task-{番号}` という名前のブランチが自動で作成されます。特定のブランチに作業を追加したい場合のみ入力してください。
...
```
[GIT] ブランチ 'karakuri/task-1' を push しています...
To git@github.com:ユーザー名/リポジトリ名.git
 * [new branch]      karakuri/task-1 -> karakuri/task-1
Branch 'karakuri/task-1' set up to track remote branch 'karakuri/task-1' from 'origin'.
[GIT] push 完了
```

GitHubのリポジトリページを開くと `karakuri/task-1` ブランチに `translator.html` が保存されています。
...
ただし、**Karakuri を再起動するとコンテナも再起動されます**。再起動後に過去のタスクのコンテナが停止している場合は、タスクを削除して作り直してください。
...
```bash
cd karakuri/backend   # クローンしたディレクトリ
source venv/bin/activate
uvicorn app.main:app --reload --host 0.0.0.0 --port 8000
```
